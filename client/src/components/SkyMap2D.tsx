...
export function SkyMap2D({ aircraft, anomalies }: SkyMap2DProps) {
  ...
  return (
    <div className="skymap-panel">
      <div className="panel-header">Radar Scope</div>
      <div className="skymap-wrapper">
        ...
        >
          ...
          {(() => {
            ...
            const runwayStart = nmToView(
              -halfW,
              -halfH,
              VIEW_WIDTH,
              VIEW_HEIGHT
            );
            const runwayEnd = nmToView(
              halfW,
              -halfH,
              VIEW_WIDTH,
              VIEW_HEIGHT
          ...
          {(() => {
            const halfW = PATTERN_WIDTH_NM / 2;
            const halfH = PATTERN_HEIGHT_NM / 2;
            const bottomLeft = nmToView(
              -halfW,
              -halfH,
              VIEW_WIDTH,
              VIEW_HEIGHT
            );
            const bottomRight = nmToView(
              halfW,
              -halfH,
              VIEW_WIDTH,
              VIEW_HEIGHT
            );
            const topRight = nmToView(
              halfW,
              halfH,
              VIEW_WIDTH,
              VIEW_HEIGHT
            );
            const topLeft = nmToView(
              -halfW,
              halfH,
              VIEW_WIDTH,
              VIEW_HEIGHT
